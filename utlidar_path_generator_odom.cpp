...
float odom_y = 0;
float odom_z = 0;
float odom_trajectory_x = 0;
float odom_trajectory_y = 0;
float error_x = 0;
float error_y = 0;
...
void path_planning_feedback(position start, position goal, position last, float odom_x, float odom_y)
{
  int point_num = 30;
  float gab_x = odom_x - odom_trajectory_x;
  float gab_y = odom_y - odom_trajectory_y;

  odom_trajectory_x = odom_x;
  odom_trajectory_y = odom_y;

  error_x = gab_x - last.x;
  error_y = gab_y - last.y;

  position feedback_goal;
  feedback_goal.x = goal.x + error_x;
  feedback_goal.y = goal.y + error_y;

  std::vector<geometry_msgs::PoseStamped> pose_stamp(point_num);

  for (int i = 0; i < point_num; i++)
  {
      pose_stamp.at(i).pose.position.x = start.x + i * ((feedback_goal.x - start.x) / 30.0);
      pose_stamp.at(i).pose.position.y = start.y + i * ((feedback_goal.y - start.y) / 30.0);
  }
...
void input_state(const std_msgs::StringConstPtr& msg)
{
  ...
  if (path_num == 1)
  {
    ...
    path_planning_feedback(zero_pos, init_pos, botton_pos, odom_x, odom_y);
  }
  else if (path_num == 2)
  {
    ROS_INFO("\n[Path 2 Start]\n");
  ...
  else if (path_num == 3)
  {
    ...
    ROS_INFO("[Odom_x:   %lf   Odom_y:   %lf]",odom_x, odom_y);
    path_planning_feedback(zero_pos, init_pos, e_botton_pos, odom_x, odom_y);
  }
  else if (path_num == 4)
  {
    ROS_INFO("\n[Path 4 Start]\n");
  ...
  else if (path_num == 5)
  {
    ...
    else
    {
      ...
      ROS_INFO("[Odom_x:   %lf   Odom_y:   %lf]",odom_x, odom_y);
      path_planning_feedback(zero_pos, last_pos, e_center_pos, odom_x, odom_y);
      path_5_on = false;
